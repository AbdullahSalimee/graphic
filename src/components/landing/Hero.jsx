...
export function RadialChart({ className }) {
  ...
  return (
    <div className={`floating-card ${className}`} style={{ padding: "16px" }}>
      <svg width="120" height="120" viewBox="0 0 120 120">
        ...
        {slices.map((s, i) => {
          const [x1, y1] = polarToCart(i * 72, s.r);
          const [x2, y2] = polarToCart(i * 72 + 72, s.r);
          return (
            <g key={i}>
              <path
                d={`M${cx},${cy} L${x1},${y1} A${s.r},${s.r} 0 0,1 ${x2},${y2} Z`}
                fill={`url(#rg${i})`}
        ...
        <circle
          ...
          strokeWidth="1.5"
        />
        <circle cx={cx} cy={cy} r="2.5" fill="#4F7FFF" />
      </svg>
    </div>
  );
}
  ...
  return (
    ...
    >
      <svg width="150" height="88" viewBox="0 0 150 88">
        ...
        <path
          d="M0,70 C18,62 32,45 50,40 C68,35 82,50 100,38 C118,28 133,22 150,15 L150,85 L0,85Z"
          fill="url(#sparkGrad1)"
        />
        <path
          d="M0,78 C22,72 38,60 58,55 C78,50 92,62 112,52 C132,42 142,36 150,30 L150,85 L0,85Z"
          fill="url(#sparkGrad2)"
        />
        <path
          d="M0,78 C22,72 38,60 58,55 C78,50 92,62 112,52 C132,42 142,36 150,30"
          fill="none"
          stroke="#34D399"
          strokeWidth="2"
          strokeLinecap="round"
        ...
        />
        <path
          d="M0,70 C18,62 32,45 50,40 C68,35 82,50 100,38 C118,28 133,22 150,15"
          fill="none"
          stroke="#4F7FFF"
          strokeWidth="2.5"
          strokeLinecap="round"
          filter="url(#lineGlow)"
        />
        <circle cx="100" cy="38" r="8" fill="#4F7FFF" opacity="0.12" />
        <circle
          cx="100"
          cy="38"
          r="4"
        ...
        <rect
          ...
          rx="5"
          fill="white"
          style={{ filter: "drop-shadow(0 2px 6px rgba(79,127,255,0.2))" }}
        />
        <text
          x="127"
          y="33"
        ...
        <text x="127" y="40" textAnchor="middle" fontSize="5.5" fill="#8899CC">
          ↑ 18.4%
        </text>
      </svg>
    </div>
...
function StatNumber({ num, suffix, prefix = "", delay }) {
...
/* ──────────────────────────────────────────────────────────
   Each chart: query label, card title, and a build fn that
   uses Plotly.react() for smooth animated transitions.
─────────────────────────────────────────────────────────── */
const CHARTS = [
  /* 1 ── Radial / Polar ─────────────────────────────────── */
  {
  ...
  /* 5 ── Heatmap ────────────────────────────────────────── */
  {
    ...
    build: (Plotly, el) => {
      ...
      // Realistic shape: peaks midday weekdays
      const peak = {
        Mon: 0.8,
        Tue: 0.9,
        Wed: 1.0,
...
/* ── Chart renderer ─────────────────────────────────────── */
function PlotlyChart({ chartIndex, animating }) {
  ...
  // Remove the cleanup useEffect that also calls purge —
  // only purge on chartIndex change, not on unmount race
...
export default function Hero({ onLaunch }) {
  ...
  return (
    <section className="relative z-10 min-h-screen flex flex-col items-center justify-center px-8 pt-32 pb-20 text-center overflow-hidden">
      {/* ── Floating charts ── */}
      <Bars3DChart className="chart-1" />
      <GlowAreaChart className="chart-2" />
      <RichDonut className="chart-4" />
      <BeautifulHeatmap className="chart-5" />
      {/* Badge */}
      <div className="animate-fade-up inline-flex items-center gap-2 border border-cyan-900/20 bg-cyan-900/8 rounded-full px-3.5 py-1.5 mb-10 font-mono text-[0.68rem] text-black tracking-widest uppercase">
        <span className="w-1.5 h-1.5 rounded-full bg-white shadow-[0_0_8px_#000000] animate-pulse-dot" />
        Now with 87 chart types · including 3D &amp; Heatmaps
      </div>
      {/* Headline */}
      <h1 className="text-7xl font-playfair font-bold leading-[0.97] text-neutral-700 tracking-tight mb-1">
        The{" "}
      ...
      <p className="font-sans text-cyan-900 max-w-[500px] leading-5 mt-6 mb-12">
        From messy data to breathtaking charts in seconds.
        <br />
        Describe what you want — AI does the rest.
      </p>
      {/* Buttons */}
      <div className="animate-fade-up-d3 flex gap-4 justify-center items-center flex-wrap">
        <a
          href="#_"
      ...
      {/* Stats row */}
      <div className="animate-fade-up-d4 flex flex-wrap justify-center mt-12 w-full max-w-[680px] border border-accent/10 rounded-2xl bg-panel/60 backdrop-blur-xl overflow-hidden">
        {STATS.map(([num, suffix, label, prefix], i) => (
          <div
            key={label}
            className={`flex-1 basis-[120px] py-6 px-4 text-center transition-colors duration-200 hover:bg-accent/8 ${i < STATS.length - 1 ? "border-r border-accent/10" : ""}`}
          >
            <div className="font-playfair font-bold text-4xl text-accent leading-none mb-1.5">
              <StatNumber
                num={num}
                suffix={suffix}
                prefix={prefix}
      ...
      {/* App preview */}
      <div
        className="animate-fade-up-d5 relative w-full max-w-[960px]"
      ...
      >
        ...
        >
          ...
          <div className="flex" style={{ height: 500 }}>
            ...
            >
              ...
              <div
                key={activeIdx}
                className="px-2.5 py-2 rounded-lg font-mono text-[0.7rem] flex items-center gap-2 overflow-hidden cycle-text"
                style={{
                  background:
            ...
            <div className="flex-1 flex flex-col p-4 gap-3 min-w-0 overflow-hidden">
              ...
              <div className="flex justify-end">
                <div
                  key={`msg-${activeIdx}`}
                  className="inline-flex items-center gap-2 rounded-2xl px-4 py-2 font-mono text-[0.78rem] text-slate-200 cycle-text max-w-xs"
                  style={{
                    background:
              ...
              >
                ...
                >
                  <div className="flex items-center gap-2.5">
                    ...
                    <span
                      key={`ttl-${activeIdx}`}
                      className="font-mono text-[0.7rem] text-slate-400 tracking-wide cycle-text"
                    >
                      {chart.title}
                    </span>
                  </div>
        ...
        <style>{`
        .cycle-text {
          animation: cycleIn 0.44s cubic-bezier(0.16,1,0.3,1) both;
        }
        @keyframes cycleIn {
