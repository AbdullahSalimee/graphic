...
/* Slide-in hook — fires once when element enters viewport */
...
function StepRow({ step }) {
  ...
  // Text always slides in from the left (or right if flipped)
  // Viz always slides in from the opposite side
  const textDir = step.flip ? "right" : "left";
  const vizDir = step.flip ? "left" : "right";
  ...
  return (
    <div
      className={`flex items-center gap-16 flex-wrap mb-22 ${step.flip ? "flex-row-reverse" : ""}`}
      style={{ marginBottom: "5.5rem" }}
    >
      {/* Text — merges both the existing useReveal ref and our slide ref */}
      <div
        ref={(el) => {
          textRef.current = el;
      ...
      >
        <div className="font-playfair font-bold text-[5.5rem] leading-none text-white mb-2 opacity-100">
          {step.n}
        </div>
        <div className="font-mono text-[0.63rem] tracking-[0.16em] uppercase text-black opacity-100 mb-5">
          Step {step.n}
        </div>
        <h3
          className="font-playfair font-bold text-neutral-700 tracking-tight leading-[1.1] mb-4"
          style={{ fontSize: "clamp(1.8rem,3.5vw,2.6rem)" }}
        >
          {step.title}
        </h3>
        <p className="font-sans text-cyan-900 leading-[1.8] max-w-[400px]">
          {step.body}
        </p>
      </div>
      ...
      <div
        ...
        className="flex-1 basis-[340px] h-[260px] rounded-2xl border border-accent/10 bg-gradient-to-br from-surface to-panel flex items-center justify-center relative overflow-hidden shadow-[0_20px_80px_rgba(0,0,0,0.5)]"
        style={slideVizStyle}
      >
  ...
  return (
    <section className="relative z-10 px-8 py-28 max-w-[1100px] mx-auto">
      <div ref={ref} className="text-center mb-20">
        <div className="font-mono text-[0.63rem] tracking-[0.16em] uppercase text-black  opacity-75 mb-5">
          How it works
        </div>
        <h2
          className="font-playfair font-bold text-neutral-700 tracking-tight leading-[1.1] mb-5"
          style={{ fontSize: "clamp(2rem,4.5vw,3.5rem)" }}
        >
          Three steps to <em className="not-italic text-accent">clarity</em>
        </h2>
        <p className="font-sans text-slate-400 leading-[1.8] max-w-[460px] mx-auto">
