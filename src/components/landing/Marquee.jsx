...
function GroupedBarChart() {
  ...
  const bW = gW * 0.32;
  const toY = (v) => pad + (1 - v / maxV) * (H - pad * 2);
...
function StackedAreaChart() {
  ...
  const W = 200,
    H = 110,
    pad = 10;
  const toX = (i) => pad + (i * (W - pad * 2)) / 10;
  const maxSum = Math.max(...xs.map((_, i) => a[i] + b[i] + c[i]));
  const toY = (v) => H - pad - (v * (H - pad * 2)) / maxSum;
...
function GaugeChart() {
  ...
  return (
    <svg width={200} height={110} viewBox="0 0 200 110">
      ...
      {/* Zone arcs */}
      {zones.map((z, i) => {
        const a1 = startA + totalA * z.from;
        const a2 = startA + totalA * z.to;
        return (
          <path
            key={i}
            d={arc(cx, cy, R, a1, a2)}
...
export default function Marquee() {
  ...
  useEffect(() => {
    ...
    // Get the width of ONE set (half of the duplicated list)
    const tick = () => {
  ...
  const items = [...CHARTS, ...CHARTS]; // duplicate for seamless loop
            ...
            <div
              ...
              onMouseEnter={(e) => {
                ...
                e.currentTarget.style.boxShadow =
                  "0 8px 40px rgba(6,182,212,0.12), 0 0 0 1px rgba(6,182,212,0.1)";
